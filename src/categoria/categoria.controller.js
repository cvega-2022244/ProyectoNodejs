...
export const agregarCategoria = async(req,res)=>{
    try {
        ...
        return res.send({message: 'Se agrego exitosamente la categoria '})
...
export const eliminarCategoria = async (req, res) => {
    try {
        ...
        if (!eliminarCategoria) return res.status(404).send({ message: 'No se encontró la categoría ' })
        return res.send({ message: ` ${eliminarCategoria.categoria} fue eliminada` })
    } catch (err) {
        console.error(err);
...
export const porDefecto = async (id, res) => {
    try {
        ...
        if (!productos || productos.length === 0) return res.status(400).send({ message: 'No se encontraron productos con esa categoría' })
